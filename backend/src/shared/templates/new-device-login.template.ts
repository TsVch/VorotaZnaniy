/**
 * HTML template for New Device Login security alert email.
 *
 * Uses a warning/attention style. Sent when a login is detected from
 * a device fingerprint that hasn't been seen in the user's recent sessions.
 *
 * @param userAgent - Browser / device user-agent string
 * @param ipAddress - IP address of the login request
 * @param timestamp - ISO date string of when the login occurred
 * @returns Complete HTML string suitable for Resend API
 */
export function renderNewDeviceLoginTemplate(
  userAgent: string,
  ipAddress: string,
  timestamp: string,
): string {
  const escapedUserAgent = userAgent
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;')
    .replace(/"/g, '&quot;');
  const escapedIp = ipAddress
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;');
  const formattedDate = new Date(timestamp).toLocaleString('ru-RU', {
    day: 'numeric',
    month: 'long',
    year: 'numeric',
    hour: '2-digit',
    minute: '2-digit',
    timeZoneName: 'short',
  });

  return `<!DOCTYPE html>
<html lang="ru" xmlns="http://www.w3.org/1999/xhtml">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Вход с нового устройства</title>
</head>
<body style="margin:0;padding:0;background-color:#f4f5f7;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Helvetica Neue',Arial,sans-serif;">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#f4f5f7;padding:40px 16px;">
    <tr>
      <td align="center">
        <table role="presentation" width="480" cellpadding="0" cellspacing="0" style="background-color:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.06);">
          <!-- Header — Orange/amber warning -->
          <tr>
            <td style="background:linear-gradient(135deg,#f59e0b,#d97706);padding:32px 40px;text-align:center;">
              <div style="font-size:48px;margin-bottom:8px;">&#128187;</div>
              <h1 style="margin:0;color:#ffffff;font-size:24px;font-weight:700;letter-spacing:-0.3px;">
                Вход с нового устройства
              </h1>
            </td>
          </tr>
          <!-- Body -->
          <tr>
            <td style="padding:32px 40px;">
              <h2 style="margin:0 0 12px;color:#1e293b;font-size:20px;font-weight:600;">
                Зафиксирован новый вход в аккаунт
              </h2>
              <p style="margin:0 0 20px;color:#64748b;font-size:15px;line-height:1.5;">
                Мы обнаружили вход в ваш аккаунт KnowledgeVault
                с ранее не использовавшегося устройства.
              </p>
              <!-- Device info card -->
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#fffbeb;border-radius:8px;padding:16px;margin-bottom:24px;border:1px solid #fde68a;">
                <tr>
                  <td style="padding-bottom:8px;">
                    <p style="margin:0 0 2px;color:#92400e;font-size:12px;font-weight:600;">Устройство</p>
                    <p style="margin:0;color:#78350f;font-size:14px;word-break:break-word;">${escapedUserAgent}</p>
                  </td>
                </tr>
                <tr>
                  <td style="padding-bottom:8px;">
                    <p style="margin:0 0 2px;color:#92400e;font-size:12px;font-weight:600;">IP-адрес</p>
                    <p style="margin:0;color:#78350f;font-size:14px;">${escapedIp}</p>
                  </td>
                </tr>
                <tr>
                  <td>
                    <p style="margin:0 0 2px;color:#92400e;font-size:12px;font-weight:600;">Время</p>
                    <p style="margin:0;color:#78350f;font-size:14px;">${formattedDate}</p>
                  </td>
                </tr>
              </table>
              <!-- Warning -->
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#fef2f2;border-radius:8px;padding:16px;margin-bottom:24px;border:1px solid #fecaca;">
                <tr>
                  <td style="vertical-align:top;width:24px;font-size:18px;">&#9888;&#65039;</td>
                  <td>
                    <p style="margin:0;color:#991b1b;font-size:14px;line-height:1.5;">
                      <strong>Если это были не вы,</strong> немедленно смените
                      пароль в настройках аккаунта.
                    </p>
                  </td>
                </tr>
              </table>
              <hr style="border:none;border-top:1px solid #e2e8f0;margin:24px 0 16px;" />
              <p style="margin:0;color:#94a3b8;font-size:12px;line-height:1.5;">
                Это автоматическое уведомление об устройстве. Если вы узнали
                это устройство, можете проигнорировать данное сообщение.
              </p>
            </td>
          </tr>
        </table>
        <p style="margin:16px 0 0;color:#94a3b8;font-size:12px;">
          KnowledgeVault &mdash; Ваша безопасная база знаний
        </p>
      </td>
    </tr>
  </table>
</body>
</html>`;
}
